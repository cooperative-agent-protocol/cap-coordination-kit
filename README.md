...
# 3. Arbiter + Handover invariants (no-double-grant, atomic transfer, monitor)
python3 -m pytest tests/ -q          # 22 passed

# 4. Re-derive the per-transition conformance verdict from a shipped campaign log
...
This kit makes the **operational-P4 choreography** and the **per-transition
conformance verdict** publicly reproducible, and lets the **arbiter and handover
invariants** be unit-tested directly. It does **not** regenerate the contended
